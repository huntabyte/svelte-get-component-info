...
function getPropType(s: string): string | undefined {
    ...
    const firstDelimiters: number =
        positionEquals > -1 ? positionEquals : positionSemicolon
    const positionRegex: number = typeRegex?.index ? typeRegex.index : -1
    const cleanedType =
        typeRegex && typeRegex[0]
            ? typeRegex[0].replace(regex.multiLine, " ").trim()
            : undefined
    const type: string | undefined =
        positionRegex < firstDelimiters && typeRegex ? cleanedType : undefined
    return type
}
...
function getPropDefaultValue(s: string): string | undefined {
    ...
    const defaultValue: string | undefined =
        positionEquals < 0
            ? undefined
            : s.substring(positionEquals + 1, positionSemicolon).trim()
    const result: string | undefined =
        defaultValue && isStringType(defaultValue)
            ? getStringWithoutQuote(defaultValue)
            : defaultValue
    return result
}
